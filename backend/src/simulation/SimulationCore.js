...
class SimulationCore {
  ...
  async assign(agent, plan, newOrder, { batchGroupId = null } = {}) {
    const second = this.clock;
    agent.plan = plan;
    agent.commitments[newOrder.external_order_number] = {
      promisedSecond: this.promisedDropoffSecond(plan, newOrder.external_order_number),
      acceptedSecond: second,
    };

    const orderNumbers = [...new Set(plan.stops.filter((s) => s.orderNumber !== null).map((s) => s.orderNumber))];
  ...
  async tryBatch(agent, order) {
    ...
    // Ruta sin el pedido nuevo, desde la misma posición y con el mismo
    // método, para medir km y minutos marginales de forma coherente.
    const currentPlan = await this.buildPlan(agent, remainingStops, second, "DELIVERY");
  ...
  routePayload(agent, second = this.clock) {
    ...
    const coordinates = [[agent.position.lng, agent.position.lat]];
    plan.legs.forEach((leg) => {
      if (leg.toSecond <= second) return;
