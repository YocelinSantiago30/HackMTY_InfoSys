...
async function getSimulation(userId, simulationId) {
  const row = await getOwnedSimulation(userId, simulationId);
  // Snapshot confirmado: permite recuperar el mapa incluso en pausa, sin
  // esperar un evento de socket ni reanudar o modificar el turno.
  const result = await pool.query("SELECT snapshot FROM simulation_runtime WHERE simulation_id = $1", [simulationId]);
  if (!result.rows[0]?.snapshot) return row;
  const SimulationCore = require("../simulation/SimulationCore");
  const core = SimulationCore.fromSnapshot({
    snapshot: result.rows[0].snapshot,
    simulationOrders: [],
    options: { simulationId, seed: Number(row.seed), durationSeconds: row.simulation_duration_seconds },
  });
  return { ...row, visualState: { ...core.snapshot(), routes: Object.fromEntries(Object.values(core.agents).map(agent => [agent.code, core.routePayload(agent)])) } };
}
