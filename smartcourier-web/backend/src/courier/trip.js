...
function planTrip(order, ctx) {
  ...
  if (inFlight.length === 0) {
    ...
    return {
      ...
      sequence: "single",
      lateOrders: orderDeadline(order) !== null && single.dropoffSeconds > orderDeadline(order) ? [order.order_id] : [],
    };
  }
  ...
  const planA = {
    ...
    load: newLoad,
    lateOrders: carried.filter(item => orderDeadline(item) !== null && now + item.remaining > orderDeadline(item)).map(item => item.order_id),
  };
  ...
  return {
    ...
    // A ya contiene SOLO el viaje nuevo: descontar baselineKm otra vez
    // subestimaba el costo y podía convertir un pedido perdedor en rentable.
    km: chosen.sequence === "finish_in_flight_first" ? chosen.km : Math.max(0, chosen.km - baselineKm),
