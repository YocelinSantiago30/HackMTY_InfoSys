...
test("el pago base debe superar el combustible aunque la propina y la estrategia prometan mucho", () => {
  for (const vehicle of ["moto", "car", "bike"]) {
    const cost = 4 * VEHICLES[vehicle].fuelMxnPerKm;
    for (const base_pay_mxn of [0, cost]) {
      const r = run(order({ vehicle, base_pay_mxn, est_tip_mxn: 1000 }), -Infinity);
      assert.equal(r.decision, "SKIP");
      assert.ok(r.economics.net_before_tip_mxn <= 0);
    }
    const profitable = run(order({ vehicle, base_pay_mxn: cost + 0.01, est_tip_mxn: 0 }), -Infinity);
    assert.equal(profitable.decision, "ACCEPT");
    assert.equal(profitable.economics.net_before_tip_mxn, 0.01);
  }
});

test("rechaza entradas que podrían convertir kilómetros o costos inválidos en ganancias", () => {
  for (const key of ["distance_pickup_km", "distance_delivery_km", "base_pay_mxn", "est_tip_mxn", "weight_kg", "volume_liters", "restaurant_prep_min"]) {
    for (const value of [-1, NaN, Infinity, "20", null]) assert.throws(() => run(order({ [key]: value })), /no negativo/);
  }
  for (const value of [0, -1, Infinity]) assert.throws(() => run(order({ surge_multiplier: value })), /mayor que cero/);
  assert.throws(() => run(order({ courier_state_overrides: { continuous_riding_min: -10 } })), /no negativos/);
  assert.throws(() => run(order({ courier_state_overrides: { in_flight_orders: [{ remaining_km: -100 }] } })), /no negativo/);
});

test("terminar el pedido anterior no descuenta dos veces los kilómetros ya comprometidos", () => {
  const make = remaining_km => order({ distance_pickup_km: 10, courier_state_overrides: { in_flight_orders: [{ order_id: "IN", zone_dropoff: 1, remaining_min: 1, remaining_km, delivery_deadline: "2026-03-21T19:00:00" }] } });
  const evaluate = request => decide(request, { demandModel, strategy: { reservation_wage_mxn_hr: 0 } });
  const a = evaluate(make(0));
  const b = evaluate(make(3));
  assert.equal(a.explain.inputs.trip.sequence, "finish_in_flight_first");
  assert.equal(b.explain.inputs.trip.sequence, "finish_in_flight_first");
  assert.ok(a.response.economics.fuel_cost_mxn > 0);
  assert.equal(a.response.economics.fuel_cost_mxn, b.response.economics.fuel_cost_mxn);
  assert.equal(a.explain.inputs.trip.km, require('../src/courier/world').zoneDistanceKm(1, 1) + 3);
});

test("el plazo de entrega también se respeta cuando el pedido viaja solo", () => {
  const r = run(order({ delivery_deadline: "2026-03-21T18:01:00", base_pay_mxn: 10000 }), 0);
  assert.equal(r.decision, "SKIP");
  assert.match(r.reason, /hora prometida/);
});
