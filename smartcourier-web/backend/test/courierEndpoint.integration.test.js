...
test("laboratorio web: autentica, usa Python real y mantiene aislado el turno de Courier", async () => {
  process.env.JWT_SECRET ||= "courier-web-integration-secret";
  const token = require('jsonwebtoken').sign({id:'web-lab-test'}, process.env.JWT_SECRET);
  const config = {headers:{Authorization:`Bearer ${token}`}};
  const before = await get('/status');
  await assert.rejects(() => post('/api/courier/evaluate', {}), e=>e.response.status===401);
  const body={order_id:'WEB-TEST',vehicle:'moto',zone_pickup:1,zone_dropoff:12,sim_time:'2026-03-21T18:00:00',distance_pickup_km:1,distance_delivery_km:3,base_pay_mxn:80,est_tip_mxn:10,surge_multiplier:1,weight_kg:2,volume_liters:6,restaurant_prep_min:5,courier_state_overrides:{shift_end_time:'2026-03-21T23:00:00',shift_elapsed_hours:2,continuous_riding_min:0,in_flight_orders:[]}};
  const good=await post('/api/courier/evaluate',body,config);
  assert.equal(good.decision,'ACCEPT');
  assert.equal(good.degraded,false);
  assert.notEqual(good.model_version,'bootstrap');
  assert.ok(good.economics.net_before_tip_mxn>0);
  const bad=await post('/api/courier/evaluate',{...body,base_pay_mxn:1,est_tip_mxn:1000},config);
  assert.equal(bad.decision,'SKIP');
  assert.match(bad.reason,/sin propina/);
  await assert.rejects(()=>post('/api/courier/evaluate',{...body,distance_pickup_km:-100},config),e=>e.response.status===400);
  const after=await get('/status');
  assert.deepEqual(after.shift,before.shift);
  assert.deepEqual(after.courier_state,before.courier_state);
  assert.equal(after.decisions_logged,before.decisions_logged);
});
