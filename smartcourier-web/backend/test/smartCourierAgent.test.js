...
test("esperar exige superar el error de muestreo; una restricción sigue vetando", () => {
  const input={order:baseOrder(),metrics:metrics(),lookahead:{acceptAdvantage:-4,waitAdvantageRequired:20}};
  assert.equal(evaluateSmartCourier(input).decision,'ACCEPT');
  assert.equal(evaluateSmartCourier({...input,lookahead:{acceptAdvantage:-30,waitAdvantageRequired:20}}).decision,'WAIT');
  assert.equal(evaluateSmartCourier({...input,metrics:metrics({netProfit:-1})}).decision,'REJECT');
});
