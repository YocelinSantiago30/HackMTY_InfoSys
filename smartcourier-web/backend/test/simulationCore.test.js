...
const { compareAcceptVsWait, DEFAULT_LOOKAHEAD } = require("../src/agents/lookaheadPlanner");
...
test("el trazo permanece fijo al avanzar, cambiar tráfico y recuperar un snapshot", async () => {
  let calls=0;
  const core=createCore(42026,{getRoute:async params=>{calls++;return estimateRoute(params);},smartPolicy:'score'});
  await core.prepare();await core.advanceTo(core.simulationOrders[0].spec.releaseSecond);
  const agent=core.agents.SMARTCOURIER;
  assert.ok(agent.plan);
  const coords=structuredClone(core.routePayload(agent).coordinates);
  const stops=structuredClone(core.routePayload(agent).displayStops);
  const position={...agent.position};
  const initialCalls=calls;
  core.clock+=30;await core.settleAll(core.clock);
  assert.notDeepEqual(agent.position,position);
  assert.deepEqual(core.routePayload(agent).coordinates,coords);
  await core.applyEvent('TRAFFIC_INCREASED',{level:'SEVERE'});
  assert.equal(calls,initialCalls,'Avance y tráfico no vuelven a consultar rutas');
  assert.deepEqual(core.routePayload(agent).coordinates,coords);
  assert.deepEqual(core.routePayload(agent).displayStops,stops);
  const restored=SimulationCore.fromSnapshot({snapshot:core.toSnapshot(),simulationOrders:core.simulationOrders,options:{simulationId:'restored',seed:42026,durationSeconds:SHIFT_SECONDS,getRoute}});
  assert.deepEqual(restored.routePayload(restored.agents.SMARTCOURIER).coordinates,coords);
});

test("el lookahead no inventa ofertas después de terminar el turno", async () => {
  const streams=[];
  class InspectCore extends SimulationCore {
    static fromSnapshot(args){streams.push(args.simulationOrders);return super.fromSnapshot(args);}
  }
  const core=new InspectCore({simulationId:'end-test',seed:42026,durationSeconds:SHIFT_SECONDS,getRoute,smartPolicy:'score'});
  await core.prepare();await core.advanceTo(core.simulationOrders[0].spec.releaseSecond);
  const agent=core.agents.SMARTCOURIER,order=core.orders.get(1);
  agent.plan=null;agent.commitments={};core.clock=SHIFT_SECONDS-30;
  const plan=await core.buildPlan(agent,[{type:'PICKUP',orderNumber:1,lat:Number(order.pickup_lat),lng:Number(order.pickup_lng),readySecond:core.clock},{type:'DROPOFF',orderNumber:1,lat:Number(order.dropoff_lat),lng:Number(order.dropoff_lng)}],core.clock,'DELIVERY');
  const result=await compareAcceptVsWait({core,order,acceptPlan:plan});
  assert.equal(result.horizonMinutes,0.5);
  assert.equal(streams.length,2*DEFAULT_LOOKAHEAD.scenarios);
  assert.ok(streams.every(stream=>stream.every(e=>e.spec.releaseSecond<=SHIFT_SECONDS)));
  assert.ok(streams.every(stream=>stream.length===0),'No hay nuevos pings en una ventana menor al intervalo de llegada');
  assert.equal(result.wait.mean,0);
});
...
test("aceptar vs esperar: simula desde el mismo estado sin mirar los pedidos reales futuros", async () => {
  ...
  assert.equal(first.scenarios, DEFAULT_LOOKAHEAD.scenarios);
